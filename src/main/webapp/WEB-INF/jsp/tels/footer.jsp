<div id="footer">
	<div class="banner">
	 	<div id="contentFooter">
			<div id="footerLogos">
				<a href="http://www.nsf.gov" title="National Science Foundation" target="_blank">
					<img src="/webapp/themes/tels/default/images/nsf-logo.png" alt="National Science Foundation logo" />
				</a>
				<a href="http://berkeley.edu" title="University of California, Berkeley" target="_blank">
					<img src="/webapp/themes/tels/default/images/ucb-logo.png" alt="UC Berkeley logo" />
				</a>
				<a href="http://concord.org" title="Concord Consortium" target="_blank">
					<img src="/webapp/themes/tels/default/images/concord-logo.png" alt="Concord Consortium Logo" />
				</a>
				<!-- <a href="http://www.telscenter.org/confluence/display/SAIL/Home" title="SAIL Technology" target="_blank"> -->
					<img src="/webapp/themes/tels/default/images/sail-logo.png" alt="SAIL Logo" />
				<!-- </a>  -->
				<a href="http://www.telscenter.org/" title="TELS Community" target="_blank">
					<img src="/webapp/themes/tels/default/images/tels-logo-small.png" alt="TELS logo" />
				</a>
			</div>
			<div id="footerText">
				<ul>
					<li class="first"><a href="/webapp/index.html"><spring:message code="footer.link5" /></a></li>
					<li><a href="/webapp/signup.html"><spring:message code="footer.link1" /></a></li>
					<li><a href="/webapp/pages/wise-advantage.html"><spring:message code="footer.link8" /></a></li>
					<li><a href="/webapp/pages/features.html"><spring:message code="footer.link6" /></a></li>
					<li class="last"><a href="/webapp/pages/teacher-tools.html"><spring:message code="footer.link7" /></a></li>
				</ul>
				<ul>
					<li class="first"><a href="/webapp/pages/teacherfaq.html"><spring:message code="footer.link10" /></a></li>
					<li><a href="/webapp/pages/gettingstarted.html"><spring:message code="footer.link9" /></a></li>
					<li class="last"><a href="/webapp/contactwisegeneral.html"><spring:message code="footer.link2" /></a></li>
					<!-- <li><a href="<c:url value="/j_spring_security_logout"/>"><spring:message code="footer.link3" /></a></li> -->
					<!-- <li class="last"><a href="credits.html"><spring:message code="footer.link4" /></a></li> -->
				</ul>
	
				<p><spring:message code="footer.legal1" /></p>
				<p><spring:message code="footer.legal2" /> <a href="http://code.google.com/p/wise4/" title="WISE4 Software" target="_blank"><spring:message code="footer.legal2a" /></a>. &copy; <spring:message code="footer.legal3" /></p>
			</div>
			<div style="clear:both; padding:0;"></div>
		</div>
	<!--    End of contentFooter -->
	</div>
</div>
